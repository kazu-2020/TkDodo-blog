...
class PreviewBlockBuilder {
  ...
  buildEpisodeBlock() {
    ...
    const { eyecatch } = this.data.episode
    ...
    dateText.textContent = `初回放送日：${this.formatWithLocale(
  ...
  buildRecipeBlock() {
    ...
    seriesEpisodeName.textContent = `${this.data.series.name}「${this.data.episode.name}」`
    ...
    cookingTimeText.textContent = `調理時間：${this.convertCookingTime(
      this.data.recipe.cookTime
    )}`
  ...
  buildEventBlock() {
    ...
    eventDateText.textContent = `${this.formatWithLocale(
      startDate,
      'yyyy/MM/dd日(E) HH:mm'
    )} ~ ${displayEndDate}`
    ...
    locationText.textContent = `${this.data.event.place} (${this.data.event.postalAddress})`
