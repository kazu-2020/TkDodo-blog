...
Rails.application.config.to_prepare do # (required for >= Rails 7 / Zeitwerk)
  ...
  token_authenticator: proc do |token, _options|
    correct_token = if Rails.env.development? || Rails.env.test?
                      'shah7zengieg7Fa3Watah7Aeveida7aiTa4phe3Eu3eegi7meemah4eejohheenguowaeR7uM3Aithi3iexohhohboot3oot3efaiChohphaem3foo7aitoongieV7Cu'
                    else
                      ENV.fetch('OKTA_AUTH_TOKEN', nil)
                    end
