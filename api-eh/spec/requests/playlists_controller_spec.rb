    ...
    context '更新通知' do
      context '更新通知が呼び出されること' do
        ...
        context 'キーワード' do
          ...
          it '追加' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, keywords: [keyword1.name, "#{keyword1.name} 2"] } }
          end
        ...
        context 'ハッシュタグ' do
          ...
          it '追加' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, hashtags: [hashtag1.name, "#{hashtag1.name} 2"] } }
          end
        ...
        context 'Citation' do
          ...
          it '追加' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, citations_attributes: [ { id: citation1.id, name: citation1.name, url: citation1.url }, { name: "#{citation1.name} 2", url: "#{citation1.url}/2" } ] } }
          end

          it '更新' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, citations_attributes: [ { id: citation1.id, name: "#{citation1.name} 2", url: citation1.url } ] } }
          end

          it '削除' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, citations_attributes: [ { id: citation1.id, _destroy: 1 } ] } }
          end
        end
      end
      ...
      context '更新通知が呼び出されれないこと' do
        ...
        context 'Citation' do
          ...
          it '変更なし' do
            put playlist_path(playlist), params: { playlist: { name: playlist.name, citations_attributes: [ { id: citation1.id, name: citation1.name, url: citation1.url } ] } }
          end
        end
      end
    end
