...
build_rails_image() {
  ...
  if [ "${image_count}" != "0" ] ; then
    # キャッシュがある場合はキャッシュも使う
    docker build --cache-from ${latest_image} -t ${rails_image_name} -f ./infra/prd/containers/ecs/rails/Dockerfile .
  else
    docker build -t ${rails_image_name} -f ./infra/prd/containers/ecs/rails/Dockerfile .
  fi
  # dir -p ~/caches/docker
