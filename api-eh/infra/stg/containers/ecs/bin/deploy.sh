...
up_web() {
  echo start up web
  # タスク定義生成
  ruby ./api-eh/infra/stg/containers/ecs/gen_task_def.rb \
    --env_file ./api-eh/infra/stg/containers/ecs/${ENV}.env.gen \
    --secrets-file ./api-eh/infra/stg/containers/ecs/secrets.yml \
    --task-definition-template ./api-eh/infra/stg/containers/ecs/task-definition-template.json | jq '.' > task-definitions.json
  ...
  ruby ./api-eh/infra/stg/containers/ecs/gen_task_def.rb \
    --env_file ./api-eh/infra/stg/containers/ecs/${ENV}.env.gen \
    --secrets-file ./api-eh/infra/stg/containers/ecs/secrets.yml \
    --task-definition-template ./api-eh/infra/stg/containers/ecs/task-definition-template-sidekiq.json | jq '.' > task-definitions-sidekiq.json
