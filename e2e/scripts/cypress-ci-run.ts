// @see https://echobind.com/post/running-cypress-tests-in-parallel
...
(async () => {
  try {
    ...
    const command = `npx cypress run --spec "$(npx --silent ts-node scripts/cypress-spec-split.ts ${totalRunners} ${thisRunner})"`;
